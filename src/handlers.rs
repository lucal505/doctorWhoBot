...
pub async fn event_handler(
    ...
    if let serenity::FullEvent::Ready { data_about_bot} = event {
        ...
        println!("[info]: Active commands: {}.", framework.options().commands.len());
    }
    ...
    if let serenity::FullEvent::Message { new_message } = event {
        ...
        //fac un scope separat ca sa dispara lock-ul dupa ce am terminat cu el
        {
            ...
            let mut curr_question_lock = data.current_question.lock()
        ...
        if is_winner {
            ...
            //update puncte
            {
                ...
                let mut points_lock = data.points.lock()
            ...
            new_message.channel_id.say(
                &ctx.http,
                format!("Correct, <@{}>! +1 point!**", user_id)
            ).await?;
        }
    }
    Ok(())
...
pub async fn trivia_loop(ctx: serenity::Context, data: Data) -> Result<(), Error> {
    let channel_id_str = std::env::var("TRIVIA_CHANNEL_ID")
    ...
    loop {
        ...
        let should_post;
        {
            ...
            let q_lock = data.current_question.lock()
        ...
        if should_post {
            ...
            if let Some(q) = new_question {
                {
                    let mut q_lock = data.current_question.lock()
